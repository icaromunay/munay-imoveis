...
export function normalizePropertyImageUrl(url: string) {
  if (!url) return url;

  if (url.startsWith('/uploads/') || url.startsWith('/api/uploads/')) {
    const suffix = url.replace(/^\/(?:api\/)?uploads\/+/, '');
    return `/api/uploads/${suffix}`.replace(/\/+/g, '/');
  }
...
export function getPropertyThumbnailUrl(url: string) {
  ...
  const normalizedUrl = normalizePropertyImageUrl(url);

  if (normalizedUrl.startsWith('/api/uploads/properties/')) {
  ...
  return optimizeImageUrl(normalizedUrl, 400, 72);
}
